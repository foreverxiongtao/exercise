...
public final class LogUtils {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = CoreUtils.getApp()
                    .getPackageManager()
                    .getPackageInfo(CoreUtils.getApp().getPackageName(), 0);
            if (pi != null) {
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && CoreUtils.getApp().getExternalCacheDir() != null)
                sDefaultDir = CoreUtils.getApp().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                sDefaultDir = CoreUtils.getApp().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
